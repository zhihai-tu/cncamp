...
```
4. loki-grafana和loki-prometheus-server修改为NodePort
```sh
cadmin@k8snode:~$ k get svc
NAME                            TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)           AGE
envoy                           NodePort    10.97.219.161    <none>        10008:32565/TCP   37d
kubernetes                      ClusterIP   10.96.0.1        <none>        443/TCP           39d
loki                            ClusterIP   10.104.210.40    <none>        3100/TCP          7h5m
loki-grafana                    NodePort    10.106.221.30    <none>        80:32733/TCP      7h5m
loki-headless                   ClusterIP   None             <none>        3100/TCP          7h5m
loki-kube-state-metrics         ClusterIP   10.105.87.141    <none>        8080/TCP          7h5m
loki-prometheus-alertmanager    ClusterIP   10.111.236.91    <none>        80/TCP            7h5m
loki-prometheus-node-exporter   ClusterIP   None             <none>        9100/TCP          7h5m
loki-prometheus-pushgateway     ClusterIP   10.98.85.77      <none>        9091/TCP          7h5m
loki-prometheus-server          NodePort    10.103.103.103   <none>        80:31994/TCP      7h5m
```
...
cadmin@k8snode:~/tuzhihai/module10$ k get po -owide
NAME                                            READY   STATUS    RESTARTS      AGE     IP                NODE      NOMINATED NODE   READINESS GATES
httpserver-7d4bbb44f5-mbvh2                     1/1     Running   0             42s     192.168.145.198   k8snode   <none>           <none>
httpserver-7d4bbb44f5-r8fjc                     1/1     Running   0             42s     192.168.145.210   k8snode   <none>           <none>
```
6. 测试服务
```sh
cadmin@k8snode:~/tuzhihai/module10$ curl 192.168.145.210:8080
<html h1>Welcome to cncamp...</html>
cadmin@k8snode:~/tuzhihai/module10$ curl 192.168.145.210:8080/healthz
<html h1>system is working... httpcode: 200 </html>
cadmin@k8snode:~/tuzhihai/module10$ curl 192.168.145.210:8080/metrics
...
### STEP4.从 Promethus 界面中查询延时指标数据
登录prometheus前台页面，输入监控指标，展现结果。
1. 观察每一个直方图的bucket里面有多少个值 (原始数据)，在查询框中输入“httpserver_execution_latency_seconds_bucket”，如图：
![httpserver_execution_latency_seconds_bucket](https://github.com/zhihai-tu/cncamp/raw/main/homework/module10/ex10/pic/prometheus-01.jpg)

2. 观察90分位的汇聚信息（高级语法），在查询框中输入“histogram_quantile(0.9, sum(rate(httpserver_execution_latency_seconds_bucket[5m])) by (le))”，如图：
![histogram_quantile(0.9, sum(rate(httpserver_execution_latency_seconds_bucket[5m])) by (le))](https://github.com/zhihai-tu/cncamp/raw/main/homework/module10/ex10/pic/prometheus-11.jpg)

### STEP5.（可选）创建一个 Grafana Dashboard 展现延时分配情况。
1. 登录Grafana前台页面，通过json文件（详见grafana-dashboard/httpserver-latency.json）导入方式，新建一个Dashboard。

2. 查看刚才新建的面板，图形化的展示了指标分布的情况，如图：
![httpserver latency](https://github.com/zhihai-tu/cncamp/raw/main/homework/module10/ex10/pic/Grafana-01.jpg)
